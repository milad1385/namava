...
export async function POST(req: NextRequest) {
  try {
    ...
    if (subCategoryTitles.length === 0) {
      return Response.json({
        movies: [],
        aiMessage: "متأسفانه هیچ ژانری یافت نشد. لطفاً دوباره تلاش کنید.",
      });
    }
...
`;
      ...
      const aiResponse = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "تو یک دستیار فیلم و سریال هستی. پاسخ‌های کوتاه و مفید بده." },
      ...
      aiMessage = aiData.choices?.[0]?.message?.content || 
        `✨ بهترین فیلم‌های ژانر "${mainGenre}": ${topMovies.map(m => m.title).join("، ")}. پیشنهاد ویژه من "${topMovies[0]?.title}" است! 🎬`;
    } catch (aiError) {
